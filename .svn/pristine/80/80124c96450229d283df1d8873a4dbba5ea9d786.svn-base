...
export class AlertService {
  ...
  onSuccess(msg) {
    this.snotifyService.success(msg.reason, msg.status, this.getConfig());
  }
  onInfo(msg) {
    this.snotifyService.info(msg.reason, msg.status, this.getConfig());
  }
  onError(msg) {
    this.snotifyService.error(msg.reason, msg.status, this.getConfig());
  }
  onWarning(msg) {
    this.snotifyService.warning(msg.reason, msg.status, this.getConfig());
  }
  onConfirmation(msg) {
    /*
    ...
    this.snotifyService.confirm(msg.reason, msg.status, {
      ...config,
      buttons: [
  ...
  onPrompt(msg) {
     ...
     */
    const {timeout, closeOnClick, ...config} = this.getConfig(); // Omit props what i don't need
    this.snotifyService.prompt(msg.reason, msg.status, {
      ...config,
      buttons: [
